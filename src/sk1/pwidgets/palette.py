...
class Palette(wal.VPanel, wal.SensitiveDrawableWidget):
    ...
    def set_palette_size(self):
        self.remove_all()
        if self.hpal:
            size = config.palette_hcell_height
        else:
            size = config.palette_vcell_width
        self.pack((size, size))
    ...
    def paint(self):
        ...
        if self.onmax:
            self.onmax(not self.position == self.maxposition)
        if self.onmin:
            self.onmin(not self.position == 0)

        if self.hpal:
            self.set_origin(x=-self.position)
        else:
            self.set_origin(y=-self.position)
    ...
    def mouse_left_up(self, point):
        ...
        if color is not None:
            if self.on_left_click:
                self.on_left_click(color)

    def mouse_right_up(self, point):
        color = self.get_color(point)
        if color is not None:
            if self.on_right_click:
                self.on_right_click(color)
